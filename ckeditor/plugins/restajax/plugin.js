 ...
 */
 ...
 /**
 * Ajax CKEDITOR code incremented with json REST API (getjson,postjson,putjson,deljson)
 */

( function() {
	CKEDITOR.plugins.add( 'restajax', {
		requires: 'xml'
	} );
	...
	CKEDITOR.restajax = ( function() {
		...
		function getjson(url, callback, getResponseFn){
			var async = !!callback;
		...
		function postjson( url, data, callback, getResponseFn ) {
			var xhr = createXMLHttpRequest();
		...
		function putjson( url, data, callback, getResponseFn ) {
			var xhr = createXMLHttpRequest();
		...
		function deljson(url, callback, getResponseFn){
			var async = !!callback;
		...
		return {
			 ...
			 *		var data = CKEDITOR.restajax.load( 'somedata.txt' );
			 *		alert( data );
			 *
			 *		// Load data asynchronously.
			 *		var data = CKEDITOR.restajax.load( 'somedata.txt', function( data ) {
			 *			alert( data );
			 *		} );
			 *
			 ...
			 *
			 *		CKEDITOR.restajax.post( 'url/post.php', 'foo=bar', null, function( data ) {
			 *			console.log( data );
			 *		} );
			 *
			 *		CKEDITOR.restajax.post( 'url/post.php', JSON.stringify( { foo: 'bar' } ), 'application/json', function( data ) {
			 *			console.log( data );
			 *		} );
			 *
			 * @since 4.4
			 ...
			 *		var xml = CKEDITOR.restajax.loadXml( 'somedata.xml' );
			 *		alert( xml.getInnerXml( '//' ) );
			 *
			 *		// Load XML asynchronously.
			 *		var data = CKEDITOR.restajax.loadXml( 'somedata.xml', function( xml ) {
			 *			alert( xml.getInnerXml( '//' ) );
			 *		} );
			 *
			 * @param {String} url The URL from which the data is loaded.
			...
			/**
			 * GET Json (= SQL Select)
			 * GET data from a URL as json.
			 *
			 *		// Load data synchronously.
			 *		var data = CKEDITOR.restajax.getjson( 'url/ArrestDB.php/Customers/CustomerID/ALFAKI' );
			 *		alert( data );
			 *
			 *		// Load data asynchronously.
			 *		var data = CKEDITOR.restajax.getjson( 'url/ArrestDB.php/Customers/CustomerID/ALFAKI', function( data ) {
			 *			alert( data );
			 *		} );
			 *
			 * @param {String} url The URL from which the data is loaded.
			...
			/**
			 * POST Json (= SQL Create)
			 * Creates an asynchronous POST `XMLHttpRequest` of the given `url`, `data` and optional `contentType`.
			 * Once the request is done, regardless if it is successful or not, the `callback` is called
			 * with `XMLHttpRequest#responseText` or `null` as an argument.
			 *
			 *      // Create record
			 *		CKEDITOR.restajax.postjson( 'url/ArrestDB.php/Customers', 
			 *									{
			 *									"CustomerID": "ALFKI",
			...
			postjson: function( url, data, callback ) {
				return postjson( url, data, callback, getResponseJson );
			},
			/**
			 * PUT Json (= SQL Update)
			 * Creates an asynchronous PUT `XMLHttpRequest` of the given `url`, `data` and optional `contentType`.
			 * Once the request is done, regardless if it is successful or not, the `callback` is called
			 * with `XMLHttpRequest#responseText` or `null` as an argument.
			 *
			 *		// Update record
			 *		CKEDITOR.restajax.putjson( 'url/ArrestDB.php/Customers/CustomerID/ALFAKI', 
			 *									{
			 *									"CustomerID": "ALFKI",
			 *									"CompanyName": "Alfreds Futterkiste",
			 *									"ContactName": "Maria Anders",
             ...
             *									"PostalCode": "12209",
             *									"Country": "Germany",
             *									"Phone": "030-0074329",   // change phone number
             * 									"Fax": "030-0076545"
			 *									}, function( data ) {
			 *			console.log( data );
			 *		} );
			...
			putjson: function( url, data, callback ) {
				return putjson( url, data, callback, getResponseJson );
			},
			/**
			 * DELETE Json (= SQL Delete)
			 *
			 *		// Delete record synchronously.
			 *		var data = CKEDITOR.restajax.deljson( 'url/ArrestDB.php/Customers/CustomerID/ALFAKI' );
			 *		alert( data );
			 *
			...
			deljson: function( url, callback ) {
				return deljson( url, callback, getResponseJson );
			}				
		};
	} )();
